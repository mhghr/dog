...
import type { ConnectionLine, MonitoringData, MonitoringStats, Probe, UserNode } from "@/entities/probe/model/types";
...
function buildProbes(
  ...
  if (agents.length > 0) {
    return agents.filter((a) => a.status === "active" || a.status === "approved").map((agent) => {
