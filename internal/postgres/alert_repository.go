...
func scanAlert(row pgx.Row) (alerting.Alert, error) {
...
func (r *AlertRepository) ListActivePolicies(ctx context.Context, monitorID string) ([]alerting.AlertPolicy, error) {
	...
	var policies []alerting.AlertPolicy
	for rows.Next() {
		var (
			p          alerting.AlertPolicy
			scopeJSON  []byte
			condJSON   []byte
			channelIDs []string
		)
		...
		if err := rows.Scan(
			&p.ID, &p.OrganizationID, &p.ProjectID, &p.Name,
...
func (r *AlertRepository) FindByDedup(ctx context.Context, dedupKey string) (alerting.Alert, error) {
	...
	if err == pgx.ErrNoRows {
		return alerting.Alert{}, domain.ErrNotFound
	}
	if err != nil {
		return alerting.Alert{}, fmt.Errorf("find alert by dedup: %w", err)
	}
	return alert, nil
}

func (r *AlertRepository) UpsertAlert(ctx context.Context, alert *alerting.Alert) error {
...
func (r *AlertRepository) ListFiring(ctx context.Context) ([]alerting.Alert, error) {
	rows, err := r.pool.Query(ctx, `
		SELECT `+alertColumns+`
		FROM alert_events
		WHERE state = 'firing'
...
func (r *AlertRepository) ListPolicies(ctx context.Context, orgID string) ([]alerting.AlertPolicy, error) {
	rows, err := r.pool.Query(ctx, `
		SELECT
			id::text,
			organization_id::text,
...
func (r *AlertRepository) CreatePolicy(ctx context.Context, policy *alerting.AlertPolicy) error {
	...
	err = r.pool.QueryRow(ctx, `
		...
		policy.OrganizationID, policy.ProjectID, policy.Name,
...
func (r *AlertRepository) ListAlerts(ctx context.Context, orgID string) ([]alerting.Alert, error) {
	rows, err := r.pool.Query(ctx, `
		SELECT `+alertColumns+`
		FROM alert_events
		WHERE organization_id = $1
...
func (r *AlertRepository) GetAlert(ctx context.Context, alertID string) (alerting.Alert, error) {
	alert, err := scanAlert(r.pool.QueryRow(ctx, `
		SELECT `+alertColumns+`
		FROM alert_events
		WHERE id = $1`, alertID))
	...
	if err != nil {
		return alerting.Alert{}, fmt.Errorf("get alert: %w", err)
	}
	return alert, nil
}

func scanAlerts(rows pgx.Rows) ([]alerting.Alert, error) {
